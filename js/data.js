...
export const DIAGNOSIS_MATRIX = {
    1: {
        autocontrol: { q: "¿Se para un momento cuando le dices 'no' con firmeza?", target: 2.0 },
        responsabilidad: { q: "¿Te ayuda un poco al vestirle (metiendo el brazo o la pierna)?", target: 1.5 },
        respeto: { q: "¿Te mira para ver si lo que está haciendo te parece bien?", target: 2.5 },
        autonomia: { q: "¿Intenta beber solo de su vaso o coger la cuchara?", target: 2.0 },
        emocional: { q: "¿Busca que le abraces para calmarse cuando se asusta?", target: 3.0 },
        social: { q: "¿Le gusta mirar o acercarse a otros niños para jugar?", target: 2.5 },
        esfuerzo: { q: "¿Sigue intentando coger un juguete que está lejos o difícil?", target: 2.0 },
        reparacion: { q: "¿Se calma rápido si le abrazas después de llorar?", target: 3.0 }
    },
    2: {
        autocontrol: { q: "¿Puede esperar unos segundos a que le des algo sin quitártelo?", target: 2.2 },
        responsabilidad: { q: "¿Te ayuda a guardar sus juguetes si tú empiezas a hacerlo?", target: 1.8 },
        respeto: { q: "¿Suelto algo que no debe si le das otro juguete a cambio?", target: 2.8 },
        autonomia: { q: "¿Intenta ponerse solo el calcetín o el gorro?", target: 2.5 },
        emocional: { q: "¿Te señala o te dice dónde le duele si se hace daño?", target: 3.2 },
        social: { q: "¿Te da un juguete un momento si se lo pides?", target: 2.8 },
        esfuerzo: { q: "¿Sigue intentando algo aunque no le salga a la primera?", target: 2.5 },
        reparacion: { q: "¿Viene a buscarte para que le mimes después de un enfado?", target: 3.5 }
    },
    3: {
        autocontrol: { q: "¿Se calma en menos de 5 minutos si le abrazas tras un enfado?", target: 2.5 },
        responsabilidad: { q: "¿Tira el pañal o un papel a la papelera si se lo pides?", target: 2.2 },
        respeto: { q: "¿Te llama por tu nombre sin gritar para que le hagas caso?", target: 3.0 },
        autonomia: { q: "¿Come casi todo solo (aunque manche un poco)?", target: 3.0 },
        emocional: { q: "¿Empieza a decir palabras como 'triste' o 'enfadado'?", target: 3.5 },
        social: { q: "¿Se acerca a otros niños para ofrecerles un juguete?", target: 3.0 },
        esfuerzo: { q: "¿Intenta terminar un puzle sencillo aunque le cueste?", target: 3.0 },
        reparacion: { q: "¿Acepta un beso o un abrazo para hacer las paces?", target: 3.8 }
    },
    4: {
        autocontrol: { q: "¿Acepta que le digas 'ahora no' sin ponerse a llorar?", target: 2.8 },
        responsabilidad: { q: "¿Recoge sus juguetes si le vas diciendo cómo hacerlo?", target: 2.5 },
        respeto: { q: "¿Dice 'por favor' y 'gracias' de vez en cuando?", target: 3.5 },
        autonomia: { q: "¿Se pone y se quita los zapatos solo (si no tienen cordones)?", target: 3.5 },
        emocional: { q: "¿Sabe decir si le duele algo o si solo está disgustado?", target: 3.8 },
        social: { q: "¿Sabe esperar un poquito su turno para jugar con otros?", target: 3.5 },
        esfuerzo: { q: "¿Termina un dibujo o una construcción que ha empezado?", target: 3.5 },
        reparacion: { q: "¿Pide perdón si le dices que ha hecho daño a alguien?", target: 4.0 }
    },
    5: {
        autocontrol: { q: "¿Para de correr o jugar fuerte cuando se lo pides?", target: 3.2 },
        responsabilidad: { q: "¿Echa su ropa sucia al cesto o guarda sus zapatos solo?", target: 3.0 },
        respeto: { q: "¿Suele escucharte sin interrumpir cuando estás hablando?", target: 3.8 },
        autonomia: { q: "¿Se viste casi todo solo (menos botones difíciles)?", target: 4.0 },
        emocional: { q: "¿Te cuenta por qué se ha enfadado cuando ya está calmado?", target: 4.0 },
        social: { q: "¿Se pone triste si ve a otro niño llorar o pasarlo mal?", target: 3.8 },
        esfuerzo: { q: "¿Sigue intentando un juego nuevo aunque no le salga?", target: 3.8 },
        reparacion: { q: "¿Intenta arreglar algo si lo ha roto o manchado?", target: 4.2 }
    },
    6: {
        autocontrol: { q: "¿Sabe esperar a que todos se sienten para empezar a comer?", target: 3.5 },
        responsabilidad: { q: "¿Prepara su mochila con tu ayuda (metiendo el agua o el pan)?", target: 3.2 },
        respeto: { q: "¿Juega limpio y acepta las reglas (sin hacer trampas)?", target: 4.0 },
        autonomia: { q: "¿Se lava o se ducha casi solo si tú le vas guiando?", target: 4.2 },
        emocional: { q: "¿Sabe calmarse un poco solo (respirando o yendo a su sitio)?", target: 4.2 },
        social: { q: "¿Tiene amigos y juega bien con ellos sin peleas seguidas?", target: 4.0 },
        esfuerzo: { q: "¿Se esfuerza por mejorar en la lectura o en algún deporte?", target: 4.0 },
        reparacion: { q: "¿Pide perdón solo si se da cuenta de que se ha equivocado?", target: 4.4 }
    },
    7: {
        autocontrol: { q: "¿Acepta perder en un juego sin enfadarse mucho?", target: 3.8 },
        responsabilidad: { q: "¿Tiene alguna tarea fija (como poner la mesa o regar)?", target: 3.5 },
        respeto: { q: "¿Habla con un tono normal en la calle o tiendas sin gritar?", target: 4.2 },
        autonomia: { q: "¿Se pone a hacer los deberes si se lo recuerdas una vez?", target: 4.4 },
        emocional: { q: "¿Te pide ayuda cuando se siente muy agobiado o triste?", target: 4.4 },
        social: { q: "¿Defiende a un amigo o hermano si alguien le trata mal?", target: 4.2 },
        esfuerzo: { q: "¿Sigue con una tarea aunque le parezca un poco aburrida?", target: 4.2 },
        reparacion: { q: "¿Busca cómo arreglar las cosas después de un error?", target: 4.5 }
    },
    8: {
        autocontrol: { q: "¿Apaga la tablet o la tele al primer aviso sin protestar?", target: 4.0 },
        responsabilidad: { q: "¿Se acuerda solo de sus deberes o de lo que tiene que llevar?", target: 3.8 },
        respeto: { q: "¿Pide permiso antes de coger algo que no es suyo?", target: 4.4 },
        autonomia: { q: "¿Hace los deberes casi siempre solo sin que estés encima?", target: 4.5 },
        emocional: { q: "¿Sabe explicar si se siente envidioso o agobiado?", target: 4.5 },
        social: { q: "¿Sabe trabajar en grupo con otros niños por un objetivo?", target: 4.4 },
        esfuerzo: { q: "¿Acepta que le digas cómo mejorar algo sin enfadarse?", target: 4.4 },
        reparacion: { q: "¿Admite su parte de culpa en una pelea con hermanos?", target: 4.6 }
    },
    9: {
        autocontrol: { q: "¿Controla sus manos y pies (no pega) cuando está tenso?", target: 4.2 },
        responsabilidad: { q: "¿Cuida bien sus cosas y sabe dónde las guarda?", target: 4.0 },
        respeto: { q: "¿Saluda y da las gracias por costumbre sin que lo digas?", target: 4.5 },
        autonomia: { q: "¿Sabe prepararse un bocadillo o algo fácil de comer?", target: 4.6 },
        emocional: { q: "¿Te dice claramente qué necesita cuando está mal?", target: 4.6 },
        social: { q: "¿Resuelve sus problemas con amigos hablando?", target: 4.5 },
        esfuerzo: { q: "¿Se pone retos (como aprender un truco) y lo intenta mucho?", target: 4.5 },
        reparacion: { q: "¿Se preocupa de que todos estén bien tras un enfado?", target: 4.7 }
    },
    10: {
        autocontrol: { q: "¿Acepta con calma que no le compres algo que quiere?", target: 4.4 },
        responsabilidad: { q: "¿Cuida su móvil o consola (si tiene) con responsabilidad?", target: 4.2 },
        respeto: { q: "¿Respeta a los profesores y abuelos sin contestar mal?", target: 4.6 },
        autonomia: { q: "¿Va solo a sitios cercanos (panadería o al parque)?", target: 4.7 },
        emocional: { q: "¿Piensa un poco en los demás y no solo en lo que él quiere?", target: 4.7 },
        social: { q: "¿Sabe decir 'no' si un amigo le propone una tontería?", target: 4.6 },
        esfuerzo: { q: "¿Entiende que hay que esforzarse para conseguir premios?", target: 4.6 },
        reparacion: { q: "¿Intenta que sus hermanos o padres se lleven bien?", target: 4.8 }
    },
    11: {
        autocontrol: { q: "¿Sabe callarse y no contestar mal si algo le molesta?", target: 4.5 },
        responsabilidad: { q: "¿Hace sus tareas de casa sin que se lo digas siempre?", target: 4.4 },
        respeto: { q: "¿Notas que valora lo que haces por él de verdad?", target: 4.7 },
        autonomia: { q: "¿Se organiza sus exámenes o entrenos con tiempo?", target: 4.8 },
        emocional: { q: "¿Se siente seguro de sí mismo y admite sus fallos?", target: 4.8 },
        social: { q: "¿Se lleva bien con gente distinta sin ser muy tímido?", target: 4.7 },
        esfuerzo: { q: "¿Sigue intentando cosas muy difíciles sin rendirse?", target: 4.7 },
        reparacion: { q: "¿Busca que haya buen ambiente en casa tras un choque?", target: 4.9 }
    },
    12: {
        autocontrol: { q: "¿Sabe discutir sin dar portazos o poner malas caras?", target: 4.6 },
        responsabilidad: { q: "¿Se ducha y tiene su cuarto ordenado sin que se lo pidas?", target: 4.5 },
        respeto: { q: "¿Respeta tus momentos de descanso sin interrumpir?", target: 4.8 },
        autonomia: { q: "¿Busca cosas nuevas que aprender por su cuenta?", target: 4.9 },
        emocional: { q: "¿Sabe separar lo que siente de lo que tiene que hacer?", target: 4.9 },
        social: { q: "¿Tiene amigos que le respetan y que son buenos para él?", target: 4.8 },
        esfuerzo: { q: "¿Se esfuerza mucho por conseguir algo que le apasiona?", target: 4.8 },
        reparacion: { q: "¿Es capaz de pedir perdón sinceramente y de corazón?", target: 5.0 }
    },
    13: {
        autocontrol: { q: "¿Usa el móvil con cabeza y sin engancharse demasiado?", target: 4.7 },
        responsabilidad: { q: "¿Entiende que las cosas valen dinero y no malgasta?", target: 4.6 },
        respeto: { q: "¿Dice lo que piensa sin insultar ni despreciar?", target: 4.9 },
        autonomia: { q: "¿Puede quedarse un rato solo cuidando de un hermano?", target: 5.0 },
        emocional: { q: "¿No se deja llevar por lo que hacen sus amigos si está mal?", target: 5.0 },
        social: { q: "¿Es un buen ejemplo o líder para sus compañeros?", target: 4.9 },
        esfuerzo: { q: "¿Trabaja por metas que tardarán mucho en llegar?", target: 4.9 },
        reparacion: { q: "¿Intenta arreglar el ambiente tras un mal día familiar?", target: 5.0 }
    },
    14: {
        autocontrol: { q: "¿Mantiene la calma aunque alguien le provoque?", target: 4.8 },
        responsabilidad: { q: "¿Acepta las consecuencias sin echar la culpa a otros?", target: 4.7 },
        respeto: { q: "¿Trata a todo el mundo con la misma educación?", target: 5.0 },
        autonomia: { q: "¿Sabe pedir cita al médico o hacer sus gestiones solo?", target: 5.0 },
        emocional: { q: "¿Acepta un fracaso sin pensar que él no vale nada?", target: 5.0 },
        social: { q: "¿Es una persona en la que sus amigos y tú confiáis?", target: 5.0 },
        esfuerzo: { q: "¿Intenta ser mejor persona cada día de forma real?", target: 5.0 },
        reparacion: { q: "¿Sabe calmar los ánimos si ve una pelea cerca?", target: 5.0 }
    },
    15: {
        autocontrol: { q: "¿Sabe esperar el momento adecuado para decir o hacer algo?", target: 4.9 },
        responsabilidad: { q: "¿Es una ayuda real en la organización de la casa?", target: 4.8 },
        resmeto: { q: "¿Su forma de hablar refleja madurez y respeto?", target: 5.0 },
        autonomia: { q: "¿Se mueve por la ciudad o viaja con total seguridad?", target: 5.0 },
        emocional: { q: "¿Se conoce bien y sabe calmarse solo si está mal?", target: 5.0 },
        social: { q: "¿Sus relaciones se basan en ayudar y ser respetado?", target: 5.0 },
        esfuerzo: { q: "¿Sabe que para ser bueno en algo hay que sacrificarse?", target: 5.0 },
        reparacion: { q: "¿Pide perdón incluso por detalles pequeños de mal tono?", target: 5.0 }
    },
    16: {
        autocontrol: { q: "¿Se domina a sí mismo casi como un adulto ya?", target: 5.0 },
        responsabilidad: { q: "¿Es totalmente de fiar cuando dice que hará algo?", target: 4.9 },
        respeto: { q: "¿Es un ejemplo de cortesía con todo el mundo?", target: 5.0 },
        autonomia: { q: "¿Lleva su vida con independencia y buen criterio?", target: 5.0 },
        emocional: { q: "¿Sabe acompañar a otros cuando están tristes o mal?", target: 5.0 },
        social: { q: "¿Se preocupa por los problemas de los demás o del mundo?", target: 5.0 },
        esfuerzo: { q: "¿Valora el trabajo bien hecho por encima de la comodidad?", target: 5.0 },
        reparacion: { q: "¿Ayuda mucho a que la familia esté unida y en paz?", target: 5.0 }
    },
    17: {
        autocontrol: { q: "¿Está tranquilo hasta en los momentos de mucho estrés?", target: 5.0 },
        responsabilidad: { q: "¿Puedes confiar en él al 100% para cualquier cosa?", target: 5.0 },
        respeto: { q: "¿Respeta a todo el mundo de forma sagrada y natural?", target: 5.0 },
        autonomia: { q: "¿Está listo para vivir solo con buenos valores?", target: 5.0 },
        emocional: { q: "¿Sabe escuchar y ayudar a los demás a estar bien?", target: 5.0 },
        social: { q: "¿Es una persona que construye cosas buenas donde va?", target: 5.0 },
        esfuerzo: { q: "¿Se esfuerza con alegría porque sabe para qué lo hace?", target: 5.0 },
        reparacion: { q: "¿Sabe perdonar de verdad y volver a empezar?", target: 5.0 }
    },
    18: {
        autocontrol: { q: "¿Es dueño de lo que hace y siente con madurez?", target: 5.0 },
        responsabilidad: { q: "¿Se hace cargo de su propia vida de forma valiente?", target: 5.0 },
        respeto: { q: "¿Es una persona exquisita en el trato y la libertad?", target: 5.0 },
        autonomia: { q: "¿Es un adulto autónomo con valores muy claros?", target: 5.0 },
        emocional: { q: "¿Tiene paz interior y sabe dársela a los demás?", target: 5.0 },
        social: { q: "¿Intenta ayudar a que el mundo sea un lugar mejor?", target: 5.0 },
        esfuerzo: { q: "¿No le asusta el sacrificio si es por algo bueno?", target: 5.0 },
        reparacion: { q: "¿Vive el perdón y la ayuda a los demás como norma?", target: 5.0 }
    }
};
